...
protected:
  ...
  void parseConfigDescr(char addr, char conf);
...
public:
  ...
  // inline uint32_t RcvData(uint16_t *bytes_rcvd, uint8_t *dataptr){ return RecvData(bytes_rcvd, dataptr); };
  // inline uint32_t RcvData(uint8_t *outBuf){ return RecvData(outBuf); };
  
  // USBDeviceConfig implementation
  virtual uint32_t Init(uint32_t parent, uint32_t port, uint32_t lowspeed);
  virtual uint32_t Release();
  virtual uint32_t Poll(){}; //not implemented
