...
uint32_t USBH_MIDI::Init(uint32_t parent, uint32_t port, uint32_t lowspeed)
{
  ...
  if( !isMidiFound ){ //MIDI Device not found. Try first Bulk transfer device
	epInfo[epDataInIndex].deviceEpNum		= epInfo[epDataInIndexVSP].deviceEpNum;
    epInfo[epDataInIndex].maxPktSize	= epInfo[epDataInIndexVSP].maxPktSize;
    epInfo[epDataOutIndex].deviceEpNum		= epInfo[epDataOutIndexVSP].deviceEpNum;
...
void USBH_MIDI::parseConfigDescr( char addr, char conf )
{
  uint8_t buf[ DESC_BUFF_SIZE ];
  uint8_t* buf_ptr = buf;
  char rcode;
...
uint32_t USBH_MIDI::RecvData(uint32_t *bytes_rcvd, uint8_t *dataptr)
{
...
/* Receive data from MIDI device */
uint32_t USBH_MIDI::RecvData(uint8_t *outBuf)
{
  char rcode = 0;     //return code
  uint32_t  rcvd;
...
uint8_t USBH_MIDI::SendData(uint8_t *dataptr, uint8_t nCable)
{
  ...
  return pUsb->outTransfer(bAddress, epInfo[epDataOutIndex].deviceEpNum, 4, buf);
}
...
uint8_t USBH_MIDI::SendSysEx(uint8_t *dataptr, unsigned int datasize, uint8_t nCable)
{
  ...
  while(n > 0) {
    ...
    rc = pUsb->outTransfer(bAddress, epInfo[epDataOutIndex].deviceEpNum, 4, buf);
    if(rc != 0)
     break;
  }
  return(rc);
